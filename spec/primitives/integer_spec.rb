...
describe DataMapper::Dynamodb::Primitive::Integer do
  let(:integer) { (Kernel.rand*100).to_i }
  let(:primitive) { DataMapper::Dynamodb::Primitive.new(integer, 'Integer') }

  context '#initializer' do
    it 'should wrap to Integer wrapper' do
      primitive.send(:dynamodb_primitive).should be_an_instance_of DataMapper::Dynamodb::Primitive::Integer
    end
  end
